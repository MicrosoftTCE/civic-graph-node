...
function drawGraph() {
    ...
    d3.json("/athena", function(error, graph) {
      ...
      function searchAutoComplete() {
        ...
        filteredNodes.forEach(function(d) {
          ...
          if (!(nickname in entitiesHash)) {
            ...
            sortedNamesList.push({id: d.id, name: d.nickname});
          }
          ...
          if(splitLocations) {
            splitLocations.forEach(function(l) {
              ...
              (!(lwcLocation in locationsHash)) ? (locationsHash[lwcLocation] = [], locationsHash[lwcLocation].push(d), sortedLocationsList.push({id: d.id, name: location})) : (locationsHash[lwcLocation].push(d));
            });
          }
        });
        ...
        //   locations.name = locations.name.toLowerCase();
        //     return locations;
        // });

        sortedSearchList = _.sortBy(sortedNamesList.concat(sortedLocationsList), function(obj) {
          return obj.name.toLowerCase();
        });

        var len = sortedSearchList.length;

        for (var count = 0; count < len; count++) {
          s += '<option value="' + sortedSearchList[count].id + '">' + sortedSearchList[count].name + '</option>';
        }
      ...
      $('#search-text').autocomplete({
        lookup: _.map(sortedSearchList, function(item){
          return item.name;
        }),
      ...
      function generateNamesDataList(sortedList) {
        ...
        for (var i = 0; i < sortedList.length; i++) {
          datalist += '<option value="' + sortedList[i].id + '">' + sortedList[i].name + '</option>';
        }
        return datalist;
      }
