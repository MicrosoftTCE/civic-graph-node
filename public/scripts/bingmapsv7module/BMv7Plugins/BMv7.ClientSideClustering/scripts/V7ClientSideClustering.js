...
var ClusteredEntityCollection = function (map, options) {
    ...
    function cluster() {
        ...
        if (_data != null) {
            ...
            var centerPixel = _map.tryLocationToPixel(new Microsoft.Maps.Location(0, 0));
            var gridBase = {
                x: centerPixel.x % _options.gridSize,
                y: centerPixel.y % _options.gridSize
            };
                ...
                if (i >= 0) {
                    do {
                        ...
                        if (pixel != null && pixel.x <= mapWidth && pixel.y <= mapHeight && pixel.x >= 0 && pixel.y >= 0) {
                            ...
                            k = Math.floor((pixel.x - gridBase.x) / _options.gridSize);
                            j = Math.floor((pixel.y - gridBase.y) / _options.gridSize);
