...
router.get('/', function(req, res) {
  ...
  var qry = select("id, name, nickname, followers, employees, entity")
...
//                           if (rows !== undefined && rows.length >= 1) {
//                           // If the entity already exists, use its ID for the entity id when inserting into connections...
//                             connection.query('INSERT INTO Bridges (entity_1_id, entity_2_id, Connection, Render, deleted_at, null) VALUES (' + result.insertId + ',' + rows[0].ID + ',"Data", 1, null);', function(err) {
//                               if (err) throw err;
...
//                                 if (err) throw err;
//                                 connection.query('INSERT INTO Bridges (entity_1_id, entity_2_id, Connection, Render) VALUES (' + result.insertId + ',' + innerRows[0].ID + ',"Data", 0, null);', function(err) {
//                                   if (err) throw err;
//                                   if(counterD === entity.data.length - 1)
//                                   {
//                                     callback(null, 'Data');
...
//                   connection.query('INSERT INTO Operations (EntityID, Finance, Amount, Year, deleted_at) VALUES (' + id + ',"Revenue",' + object.amount + ',' + object.year + ', null);', function(err) {
//                       if (err) throw err;
...
//                   connection.query('INSERT INTO Operations (EntityID, Finance, Amount, Year, deleted_at) VALUES (' + id + ',"Expenses",' + object.amount + ',' + object.year + ', null);', function(err) {
//                       if (err) throw err;
...
//       connection.query("SELECT * FROM entities ORDER BY id DESC LIMIT 0, 1", function(err, rows) {
//             if (err) throw err;

//             var newRowId = rows[0].id + 1;
...
//                         if(entity_typeof(rows[0].Followers) === "number"){
